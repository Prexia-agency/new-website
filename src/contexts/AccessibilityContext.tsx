...
export function AccessibilityProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateSetting = useCallback(<K extends keyof AccessibilitySettings>(
    ...
    setSettings(prev => {
      ...
      const readableValue = typeof value === 'boolean' 
  ...
  useEffect(() => {
    const handleKeyboard = (e: KeyboardEvent) => {
      ...
      if (e.altKey && (e.key === 'a' || e.key === 'א')) {
        e.preventDefault()
        const widget = document.querySelector('.accessibility-toggle') as HTMLElement
        widget?.click()
      }
      ...
      if (e.altKey && ['1', '2', '3', '4', '5'].includes(e.key)) {
        e.preventDefault()
        const sizes: AccessibilitySettings['fontSize'][] = ['small', 'normal', 'large', 'extra-large', 'jumbo']
      ...
      if (e.altKey && (e.key === 'd' || e.key === 'ח')) {
        e.preventDefault()
        updateSetting('contrast', settings.contrast === 'dark' ? 'normal' : 'dark')
      }
    }
...
function validateSettings(settings: Record<string, unknown>): AccessibilitySettings {
  const validOptions = {
    fontSize: ['small', 'normal', 'large', 'extra-large', 'jumbo'],
    fontFamily: ['default', 'dyslexic', 'mono'],
    contrast: ['normal', 'high', 'dark', 'light'],
  ...
  Object.keys(settings).forEach(key => {
    if (key in defaultSettings) {
      ...
      if (typeof defaultSettings[typedKey] === 'boolean') {
        (validated as Record<string, unknown>)[typedKey] = Boolean(currentValue)
      } else if (typedKey in validOptions && 
                 (validOptions as Record<string, string[]>)[typedKey].includes(currentValue as string)) {
...
function applySettings(settings: AccessibilitySettings) {
  ...
  if (typeof window !== 'undefined' && !window.document.documentElement.hasAttribute('data-hydration-complete')) {
    return
  }
