...
export const contactSchema = z.object({
  ...
  phone: z
    ...
    .refine(
      (val) => {
        ...
        const phoneRegex =
          /^(?:\+972|0)?(?:5[0-9]|[2-4]|[8-9]|7[0-9])[0-9]{7}$/;
  ...
  projectType: z
    ...
    .refine(
      (val) =>
        ["website", "mobile", "web", "ai", "branding", "other"].includes(val),
...
export const validateField = (field: keyof ContactFormData, value: unknown) => {
  try {
    // Safe field access using ternary guards
    const fieldSchema =
      field === "name"
        ...
        : field === "email"
          ...
          : field === "phone"
            ...
            : field === "company"
              ? contactSchema.shape.company
              : field === "projectType"
                ? contactSchema.shape.projectType
                : field === "message"
                  ? contactSchema.shape.message
                  : contactSchema.shape.name; // fallback
