...
export default function AccessibilityWidget() {
  ...
  const { settings, updateSetting, resetSettings, announcement } = useAccessibility()
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const firstInput = panelRef.current?.querySelector('button, input, select, [tabindex]:not([tabindex="-1"])') as HTMLElement
      firstInput?.focus()
  ...
  return (
    <>
      <div className="accessibility-widget" role="region" aria-label="בקרי נגישות">
        {/* Toggle Button */}
        <button
          ref={toggleRef}
        ...
        >
          ...
          >
            ...
            <circle cx="12" cy="17.5" r="4.5" fill="none" stroke="currentColor" strokeWidth="1.5"/>
          </svg>
          ...
          <>
            ...
            >
              <div className="accessibility-header">
                ...
                <p id="accessibility-description" className="accessibility-description">
              ...
              <div className="accessibility-content">
                ...
                <fieldset className="control-group">
                  ...
                  <div className="button-group" role="group" aria-label="אפשרויות גודל גופן">
                    {[
                      { key: 'small', label: 'קטן', shortcut: 'Alt+1' },
                      { key: 'normal', label: 'רגיל', shortcut: 'Alt+2' },
                      { key: 'large', label: 'גדול', shortcut: 'Alt+3' },
                      { key: 'extra-large', label: 'גדול מאוד', shortcut: 'Alt+4' },
                      { key: 'jumbo', label: 'ענק', shortcut: 'Alt+5' }
                    ].map(({ key, label, shortcut }) => (
                      <button
                        key={key}
                        className={`control-btn ${settings.fontSize === key ? 'active' : ''}`}
                        onClick={() => updateSetting('fontSize', key as 'small' | 'normal' | 'large' | 'extra-large' | 'jumbo')}
                        aria-pressed={settings.fontSize === key}
                        title={`גודל גופן ${label} (${shortcut})`}
                      >
                        <span className={`font-preview font-${key}`} aria-hidden="true">א</span>
                        <span className="btn-label">{label}</span>
                        {settings.fontSize === key && (
                          <span className="current-indicator" aria-label="בחירה נוכחית">✓</span>
                        )}
                      </button>
                    ))}
                  </div>
                ...
                <fieldset className="control-group">
                  ...
                  <select
                    ...
                    onChange={(e) => updateSetting('fontFamily', e.target.value as 'default' | 'dyslexic' | 'mono')}
                    className="select-control"
                    aria-describedby="font-family-help"
                  >
                    <option value="default">גופן ברירת מחדל</option>
                    <option value="dyslexic">ידידותי לדיסלקטים (OpenDyslexic)</option>
                    <option value="mono">רוחב קבוע (Monospace)</option>
                  </select>
                  <div id="font-family-help" className="help-text">
                    גופנים ידידותיים לדיסלקטים יכולים לשפר את הקריאות עבור חלק מהמשתמשים
                  </div>
                </fieldset>
                ...
                <fieldset className="control-group">
                  ...
                  <div className="button-group" role="group" aria-label="אפשרויות ערכת נושא וניגודיות">
                    {[
                      { key: 'normal', label: 'רגיל', description: 'צבעים סטנדרטיים' },
                      { key: 'high', label: 'ניגודיות גבוהה', description: 'ניגודיות גבוהה שחור וצהוב' },
                      { key: 'dark', label: 'מצב כהה', description: 'רקע כהה עם טקסט בהיר' },
                      { key: 'light', label: 'מצב בהיר', description: 'רקע בהיר עם טקסט כהה' }
                    ].map(({ key, label, description }) => (
                      <button
                        key={key}
                        className={`control-btn theme-btn theme-${key} ${settings.contrast === key ? 'active' : ''}`}
                        onClick={() => updateSetting('contrast', key as 'normal' | 'high' | 'dark' | 'light')}
                        aria-pressed={settings.contrast === key}
                        aria-describedby={`theme-${key}-desc`}
                      >
                        <span className="theme-preview" aria-hidden="true"></span>
                        <span className="btn-label">{label}</span>
                        {settings.contrast === key && (
                          <span className="current-indicator" aria-label="בחירה נוכחית">✓</span>
                        )}
                        <span id={`theme-${key}-desc`} className="sr-only">{description}</span>
                      </button>
                    ))}
                  </div>
                </fieldset>
                ...
                <fieldset className="control-group">
                  ...
                  <select
                    ...
                    onChange={(e) => updateSetting('colorBlindness', e.target.value as 'none' | 'protanopia' | 'deuteranopia' | 'tritanopia')}
                    className="select-control"
                    aria-describedby="color-blindness-help"
                  >
                    <option value="none">ללא פילטר</option>
                    <option value="protanopia">עיוורון לאדום (Protanopia)</option>
                    <option value="deuteranopia">עיוורון לירוק (Deuteranopia)</option>
                    <option value="tritanopia">עיוורון לכחול (Tritanopia)</option>
                  </select>
                ...
                <fieldset className="control-group">
                  ...
                  <div className="button-group" role="group" aria-label="אפשרויות מרווח טקסט">
                    {[
                      { key: 'normal', label: 'רגיל', description: 'מרווח סטנדרטי' },
                      { key: 'wide', label: 'רחב', description: 'מרווח מוגדל בין אותיות ושורות' },
                      { key: 'extra-wide', label: 'רחב מאוד', description: 'מרווח מקסימלי לקריאות' }
                    ].map(({ key, label, description }) => (
                      <button
                        key={key}
                        className={`control-btn ${settings.spacing === key ? 'active' : ''}`}
                        onClick={() => updateSetting('spacing', key as 'normal' | 'wide' | 'extra-wide')}
                  ...
                  <div className="toggle-controls">
                    <div className="toggle-control">
                      <input
                        ...
                        onChange={(e) => updateSetting('focusRing', e.target.checked)}
                ...
                <fieldset className="control-group">
                  ...
                  <div className="button-group" role="group" aria-label="אפשרויות גודל סמן">
                    {[
                      ...
                      <button
                        ...
                        onClick={() => updateSetting('cursor', key as 'normal' | 'large' | 'extra-large')}
