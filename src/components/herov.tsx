...
export default function HeroV() {
  ...
  const handleScrollUpdate = (self: ScrollTrigger) => {
  ...
  const updateCanvasSize = () => {
    ...
    const fallbackFrame = lastRenderedFrameRef.current ?? currentFrameRef.current.contents ?? 1;
    ...
    const ctx = gsap.context((self) => {
      ...
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
            ...
            const targetFrame = Math.max(1, Math.min(TOTAL_FRAMES, self.progress * (TOTAL_FRAMES - 1) + 1));
            ...
            const preloadEnd = Math.min(TOTAL_FRAMES, currentFrameInt + PRELOAD_AMOUNT);
