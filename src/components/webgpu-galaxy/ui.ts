...
export interface GalaxyUICallbacks {
  onUniformChange: (key: string, value: any) => void;
  onBloomChange: (property: string, value: number) => void;
  onStarCountChange: (newCount: number) => void;
  onCloudCountChange: (newCount: number) => void;
  onCloudTintChange: (color: string) => void;
  onRegenerate: () => void;
}

export class GalaxyUI {
  private pane: Pane;
  private bloomPassNode: any = null;
  private perfParams = { fps: 60 };

  constructor(
    private config: GalaxyConfig,
    private callbacks: GalaxyUICallbacks,
  ) {
  ...
  private setupPerformanceFolder() {
    ...
    // Star count control
    perfFolder
      .addBinding(this.config, "starCount", {
        min: 1000,
        max: 1000000,
        step: 1000,
        label: "Star Count",
  ...
  private setupAppearanceFolder() {
    const appearanceFolder = this.pane.addFolder({ title: "Appearance" });

    appearanceFolder
      .addBinding(this.config, "particleSize", {
        min: 0.05,
        max: 0.5,
        step: 0.01,
        label: "Star Size",
      ...
      .on("change", () =>
        this.callbacks.onUniformChange(
          "particleSize",
    ...
    appearanceFolder
      .addBinding(this.config, "starBrightness", {
        min: 0.0,
        max: 2.0,
        step: 0.01,
      ...
      .on("change", () =>
        this.callbacks.onUniformChange(
          "starBrightness",
          this.config.starBrightness,
        ),
      );

    appearanceFolder
      .addBinding(this.config, "denseStarColor", {
        label: "Dense Color",
        view: "color",
      })
      .on("change", () =>
        this.callbacks.onUniformChange(
          "denseStarColor",
          this.config.denseStarColor,
        ),
      );

    appearanceFolder
      .addBinding(this.config, "sparseStarColor", {
        label: "Sparse Color",
        view: "color",
      })
      .on("change", () =>
        this.callbacks.onUniformChange(
          "sparseStarColor",
          this.config.sparseStarColor,
        ),
      );
  }

  private setupCloudsFolder() {
    const cloudsFolder = this.pane.addFolder({ title: "Clouds" });

    cloudsFolder
      .addBinding(this.config, "cloudCount", {
        min: 0,
        max: 100000,
        step: 1000,
        label: "Count",
    ...
    cloudsFolder
      .addBinding(this.config, "cloudSize", {
        min: 0.5,
        max: 10.0,
        step: 0.01,
        label: "Size",
    ...
    cloudsFolder
      .addBinding(this.config, "cloudOpacity", {
        min: 0.0,
        max: 1.0,
        step: 0.01,
        label: "Opacity",
      ...
      .on("change", () =>
        this.callbacks.onUniformChange(
          "cloudOpacity",
          this.config.cloudOpacity,
        ),
      );

    cloudsFolder
      .addBinding(this.config, "cloudTintColor", {
        label: "Tint Color",
        view: "color",
      })
      .on("change", () =>
  ...
  private setupBloomFolder() {
    const bloomFolder = this.pane.addFolder({ title: "Bloom" });

    bloomFolder
      .addBinding(this.config, "bloomStrength", {
        min: 0,
        max: 3,
        step: 0.01,
        label: "Strength",
    ...
    bloomFolder
      .addBinding(this.config, "bloomRadius", {
        min: 0,
        max: 1,
        step: 0.01,
        label: "Radius",
    ...
    bloomFolder
      .addBinding(this.config, "bloomThreshold", {
        min: 0,
        max: 1,
        step: 0.01,
        label: "Threshold",
  ...
  private setupGalaxyFolder() {
    ...
    galaxyFolder
      .addBinding(this.config, "spiralTightness", {
        min: 0,
        max: 10,
        step: 0.01,
    ...
    galaxyFolder
      .addBinding(this.config, "armCount", {
        min: 1,
        max: 4,
        step: 1,
        label: "Arm Count",
    ...
    galaxyFolder
      .addBinding(this.config, "armWidth", {
        min: 1,
        max: 5,
        step: 0.01,
        label: "Arm Width",
    ...
    galaxyFolder
      .addBinding(this.config, "randomness", {
        min: 0,
        max: 5,
        step: 0.01,
        label: "Randomness",
    ...
    galaxyFolder
      .addBinding(this.config, "galaxyRadius", {
        min: 5,
        max: 20,
        step: 0.01,
        label: "Galaxy Radius",
    ...
    galaxyFolder
      .addBinding(this.config, "galaxyThickness", {
        min: 0.1,
        max: 10,
        step: 0.01,
        label: "Thickness",
  ...
  private setupMouseFolder() {
    ...
    mouseFolder
      .addBinding(this.config, "mouseRadius", {
        min: 1,
        max: 15,
        step: 0.01,
        label: "Radius",
