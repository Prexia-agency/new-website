...
const SecondSectionGsap = () => {
  ...
  useEffect(() => {
    ...
    const stackSection = document.querySelector(
      "[data-section='stack']",
    ) as HTMLElement | null;
    const sitesSection = document.querySelector(
      "section.bg-white",
    ) as HTMLElement | null;
  ...
  useEffect(() => {
    ...
    mm.add(
      ...
      (context) => {
        const { conditions } = context as {
          conditions: { isMobile?: boolean; isDesktop?: boolean };
        };
          ...
          <div className="max-w-6xl mx-auto px-8 w-full">
            <div className="text-center mb-1" dir="rtl" ref={contentRef}>
              ...
              <p className="text-[11px] lg:text-[18px] text-[#fffce1]/90 max-w-sm lg:max-w-2xl mx-auto">
                הלקוחות שלך גולשים היום בעיקר מהמובייל, אבל עדיין צריכים אתר
                שנראה מצוין גם על מסך מחשב. אצלנו כל חלק מקבל עיצוב מותאם אישית
                לכל פלטפורמה – כדי להבטיח חוויית משתמש חלקה, מהירה ומדויקת בכל
                גודל מסך
              </p>
            </div>
...
export default SecondSectionGsap;
